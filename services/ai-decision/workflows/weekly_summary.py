"""
Weekly Stock Summary Workflow
Generate detailed weekly summaries for all stocks and ETFs and store in stock_summaries (summary_type=weekly).
"""
...
class WeeklySummaryWorkflow:
    ...
    def __init__(
        ...
        if self.test_mode:
            logger.info("Running WeeklySummaryWorkflow in TEST MODE (no DB/RAG writes)")

    def run(self, agent_id: str) -> bool:
        """
        Execute weekly summary workflow
        ...
        """
        logger.info(
            f"Starting weekly summary for {agent_id} (test_mode={self.test_mode})",
            extra={'details': {'workflow': 'weekly_summary', 'agent_id': agent_id, 'test_mode': self.test_mode}}
        )

        try:
            # 1. Collect data
            logger.info("Step 1: Collecting weekly data")
            data = self._collect_weekly_data(agent_id)

            if not data:
                logger.error("Failed to collect weekly data")
                return False

            # 2. Generate weekly stock summaries
            logger.info("Step 2: Generating weekly stock summaries")
            stock_summaries = self._generate_weekly_stock_summaries(agent_id, data)

            if not stock_summaries:
                logger.warning("No weekly stock summaries generated")
                return False

            if self.test_mode:
                logger.info(f"TEST MODE: Generated {len(stock_summaries)} weekly stock summaries (no writes)")
                return True

            # 3. Save to database
            logger.info("Step 3: Saving weekly stock summaries to database")
            self._save_stock_summaries(agent_id, stock_summaries)

            # 4. Index to RAG
            logger.info("Step 4: Indexing weekly stock summaries to RAG")
            self._index_stock_summaries_to_rag(agent_id, stock_summaries)

            logger.info(
                f"Weekly summary completed for {agent_id}",
            ...
            logger.error(
                f"Weekly summary workflow failed: {e}",
                extra={'details': {'agent_id': agent_id}}
            )
            return False

    def _collect_weekly_data(self, agent_id: str) -> Optional[Dict[str, Any]]:
        """
        Collect data required for weekly summaries
        ...
        try:
            ...
            news = self.data_collector.collect_news(hours=lookback_hours)
    ...
    def _generate_weekly_stock_summaries(
        self,
        agent_id: str,
        data: Dict[str, Any]
    ) -> List[Dict[str, Any]]:
        """
        Generate weekly summaries for all stocks and ETFs
        """
        ...
        for symbol in data['symbols']:
            try:
                ...
                # Sort to surface most confident / recent items
                ...
                financial_reports = self.data_collector.collect_financial_reports(symbol, limit=1)
                ...
                # Weekly summary 输入量较大，使用 2 倍 timeout
                ...
                logger.info(f"Generated weekly summary for {symbol} ({len(response_text)} chars)")

            except Exception as e:
                logger.error(f"Failed to generate weekly summary for {symbol}: {e}")
                continue
    ...
    def _get_system_prompt(self) -> str:
        """
        System prompt for weekly stock summaries
        """
        return """You are a professional portfolio manager writing WEEKLY stock summaries.

For each stock/ETF, produce a detailed yet concise weekly recap (200-300 words) that includes:
1) Key news headlines from the last 7 days with your own interpretation
2) Synthesis of your latest weekly summary and past 7 daily summaries
3) Any newly available financial report insights (flag if not yet reviewed)
4) Relevant memories retrieved from your knowledge base (RAG)
5) Your stance and action bias for the coming week

Principles:
- Be analytic, consistent with prior views, and explain what changed
...
- Output plain text paragraphs only (no lists or markdown)."""
    ...
    def _build_stock_prompt(
        ...
        """
        Build per-stock prompt
        """
        parts = [f"# Weekly Summary Input for {symbol}", ""]
        ...
        parts.append("## Financial Reports (latest)")
        if financial_reports:
            fr = financial_reports[0]
            parts.append(
                f"{fr.get('report_type', 'N/A')} FY{fr.get('fiscal_year', 'N/A')} "
                f"Q{fr.get('fiscal_quarter', 'N/A')} filed {fr.get('filing_date')}"
            )
            if fr.get('summary_en'):
                parts.append(f"Summary: {fr['summary_en'][:600]}")
            else:
                parts.append("No extracted summary yet.")
        else:
            parts.append("No completed financial reports found.")
        parts.append("")
        ...
        parts.append("## Task")
        parts.append(
            "Write a weekly recap (200-300 words). Blend news, your past summaries, financial reports, and memories. "
            "State conviction and next-week bias clearly. Output plain text only."
        )
    ...
    def _save_stock_summaries(
        ...
        """
        Save weekly stock summaries to database
        """
        try:
            query = """
            ...
            logger.info(f"Saved {len(stock_summaries)} weekly stock summaries to database")
            return True

        except Exception as e:
            logger.error(f"Failed to save weekly stock summaries: {e}")
            return False
    ...
    def _index_stock_summaries_to_rag(
        ...
        """
        Index weekly stock summaries to OpenSearch RAG
        """
        try:
            for summary in stock_summaries:
            ...
            logger.info(f"Indexed {len(stock_summaries)} weekly stock summaries to RAG")
            return True

        except Exception as e:
            logger.error(f"Failed to index weekly stock summaries to RAG: {e}")
            return False
